...
4. If the task involves UI work:
   a. Detect the project stack from project files
   b. Check if the matching Syncfusion UI Builder is installed (per-project dependency)
   c. If installed → route to the UI Builder agent
   d. If NOT installed → inform user with install command (`apm install syncfusion/<framework>-ui-builder -t <target>`), fall back to `incremental-implementation` + `frontend-ui-engineering`
